...
В случае возникновения проблем с запуском просьба обратиться к автору работы.

# Задание 2

Представленный монолитный сервис содержит большое количество функций, многие из которых явно относятся к какому-либо домену. Поэтому предлагается разбить сервис на несколько отдельных микросервисов, как на схеме по ссылке ниже:

[Ссылка на draw.io в github](./sprint_1_2.drawio)

[Ссылка на png в github для быстрого просмотра](./sprint_1_2.drawio.png)

Замечу, что пользовательские потоки обозначены однонаправленными, чтобы не перегружать схему, но по факту в большинстве случаев они являются двунаправленными, где это логично (запрос - ответ с данными).
